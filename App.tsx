...
import { LayoutDashboard, CheckSquare, PlusCircle, Settings, LogOut, Wallet, Menu, X } from 'lucide-react';
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-20 md:pb-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div className="w-full md:w-auto">
           ...
           <p className="text-gray-500 mt-1 text-sm md:text-base">Manage your active portfolio</p>
        </div>
        <div className="w-full md:w-auto bg-white px-4 md:px-6 py-3 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between md:justify-start gap-4">
            <div>
                <p className="text-[10px] md:text-xs text-gray-500 uppercase">Total Outstanding</p>
                <p className="text-lg md:text-xl font-bold text-blue-600">{formatCurrency(totalOutstanding)}</p>
            </div>
            <div className="h-8 w-px bg-gray-200"></div>
            <div>
                 <p className="text-[10px] md:text-xs text-gray-500 uppercase">Active Loans</p>
                 <p className="text-lg md:text-xl font-bold text-gray-800 text-right md:text-left">{activeLoans.length}</p>
            </div>
        </div>
      </div>
      ...
      {filteredLoans.length === 0 ? (
          <div className="text-center py-12 md:py-20 bg-white rounded-2xl border border-dashed border-gray-300 mx-auto">
             ...
             <h3 className="text-base md:text-lg font-medium text-gray-900">No active loans found</h3>
             <p className="text-sm md:text-base text-gray-500 max-w-sm mx-auto mt-2 px-4">Create a new loan application to get started or adjust your search.</p>
          </div>
      ) : (
...
const HistoryPage = ({ onSelectLoan }: { onSelectLoan: (id: string) => void }) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20 md:pb-0">
             <div>
                <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Loan History</h1>
                <p className="text-gray-500 mt-1 text-sm md:text-base">Archive of completed and closed loans.</p>
             </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-30 bg-slate-900/95 backdrop-blur-sm pt-20 px-4 animate-in slide-in-from-top-5 duration-200">
           <nav className="space-y-2">
             <NavItem to="/" icon={LayoutDashboard} label="Active Loans" active={location.pathname === '/' || location.pathname.startsWith('/loan')} onClick={() => setIsMobileMenuOpen(false)} />
             <NavItem to="/history" icon={CheckSquare} label="Completed History" active={location.pathname === '/history'} onClick={() => setIsMobileMenuOpen(false)} />
           </nav>
        </div>
      )}
