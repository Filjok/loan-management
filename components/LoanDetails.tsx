...
const LoanDetails: React.FC<LoanDetailsProps> = ({ loanId, onBack }) => {
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      <button onClick={onBack} className="flex items-center text-gray-500 hover:text-gray-900 transition-colors py-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex justify-between items-start">
             <div className="overflow-hidden">
                <h2 className="text-xl md:text-2xl font-bold text-gray-900 truncate">{loan.borrower.name}</h2>
                <p className="text-gray-500 text-sm mt-1 truncate">{loan.borrower.address}</p>
             </div>
          ...
          <div className="mt-6 space-y-2">
            <div className="flex justify-between text-sm">
                ...
                <span className="font-medium truncate ml-2">{loan.borrower.idProof}</span>
            </div>
            <div className="flex justify-between text-sm">
        ...
        <div className="bg-slate-900 text-white p-5 md:p-6 rounded-2xl shadow-lg relative overflow-hidden">
            ...
            <p className="text-slate-400 text-xs md:text-sm uppercase font-semibold">Current Outstanding</p>
            <h1 className="text-3xl md:text-4xl font-bold mt-2">{formatCurrency(loan.currentBalance)}</h1>
            
            <div className="mt-6 grid grid-cols-2 gap-4">
                <div>
                    <p className="text-slate-400 text-xs">Principal</p>
                    <p className="font-medium text-sm md:text-base">{formatCurrency(loan.principalAmount)}</p>
                </div>
                <div>
                    <p className="text-slate-400 text-xs">Rate (Monthly)</p>
                    <p className="font-medium text-sm md:text-base">{loan.interestRate.toFixed(2)}%</p>
                </div>
            </div>
        </div>
               ...
               <form onSubmit={handlePayment} className="space-y-4">
                 ...
                 <div className="bg-emerald-50 p-3 rounded-lg border border-emerald-100 text-sm text-emerald-800">
                    <div className="flex justify-between items-center">
                        <span className="text-xs md:text-sm">Interest ({diffDays} days):</span>
                        <span className="font-bold text-base md:text-lg">{formatCurrency(expectedInterest)}</span>
                    </div>
                    <div className="text-[10px] md:text-xs text-emerald-600 mt-1">Since: {lastDate.toLocaleDateString()}</div>
                 </div>
                 ...
                 <button type="submit" className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors shadow-sm active:transform active:scale-95">
                    Add Payment
                 </button>
               </form>
           ) : (
      ...
      {/* AI Analysis Section */}
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 border border-blue-100 rounded-2xl p-5 md:p-6 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
            ...
            {!analysis && (
                <button 
                    ...
                    className="w-full sm:w-auto px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors"
                >
            ...
            <div className="space-y-4 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                    <div className="bg-white/60 p-3 rounded-lg border border-indigo-100">
                        <p className="text-[10px] md:text-xs text-indigo-500 uppercase font-bold">Financial Health</p>
                    ...
                    <div className="bg-white/60 p-3 rounded-lg border border-indigo-100">
                        <p className="text-[10px] md:text-xs text-indigo-500 uppercase font-bold">Projected Finish</p>
                        <p className="font-bold text-gray-800 text-sm md:text-base">{analysis.projectedFinishDate || "N/A"}</p>
                    </div>
                </div>
                <div>
                    <p className="text-[10px] md:text-xs text-indigo-500 uppercase font-bold mb-2">Recommendations</p>
